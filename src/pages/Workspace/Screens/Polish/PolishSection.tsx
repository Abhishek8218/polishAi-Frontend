...
export default function PolishSection() {
  ...
  const polishMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      // Switch to output tab on mobile after polishing
      setMobileTab("output");
    },
    onError: (error: any) => {
      ...
      // Switch to output tab on mobile to show the error
      setMobileTab("output");
    },
  });
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden h-full">
      ...
      <div className="flex lg:hidden border-b border-[#2a2a2e] bg-[#111110] shrink-0">
        <button
          onClick={() => setMobileTab("input")}
          className={`flex-1 py-3 text-[12px] font-semibold tracking-widest uppercase transition-colors ${
            mobileTab === "input"
              ? "text-white border-b-2 border-[#0d8a7c]"
              : "text-[#5c5c6e]"
          }`}
        >
          Input
        ...
        <button
          ...
          className={`flex-1 py-3 text-[12px] font-semibold tracking-widest uppercase transition-colors relative ${
            mobileTab === "output"
              ? "text-white border-b-2 border-[#0d8a7c]"
              : "text-[#5c5c6e]"
          }`}
        >
          Output
          {messages.length > 0 && mobileTab !== "output" && (
            <span className="absolute top-2 right-6 w-2 h-2 rounded-full bg-[#0d8a7c]" />
          )}
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div
          className={`
            flex flex-col w-full lg:w-[55%] lg:border-r border-[#2a2a2e] bg-[#111110] overflow-y-auto
        ...
        <div
          className={`
            flex flex-col flex-1 bg-[#0e0e0f] overflow-hidden
